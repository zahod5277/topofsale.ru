...
path=path.split('.');var obj=miniShop2.Callbacks;for(var i=0;i<path.length;i++){if(obj[path[i]]==undefined){return!1}
obj=obj[path[i]]}
if(typeof obj!='object'){obj=[obj]}
if(name!=undefined){obj[name]=func}
else{obj.push(func)}
return!0};miniShop2.Callbacks.remove=function(path,name){path=path.split('.');var obj=miniShop2.Callbacks;for(var i=0;i<path.length;i++){if(obj[path[i]]==undefined){return!1}
obj=obj[path[i]]}
if(obj[name]!=undefined){delete obj[name];return!0}
return!1};miniShop2.ajaxProgress=!1;miniShop2.setup=function(){this.actionName='ms2_action';this.action=':submit[name='+this.actionName+']';this.form='.ms2_form';this.$doc=$(document);this.sendData={$form:null,action:null,formData:null}};miniShop2.initialize=function(){miniShop2.setup();miniShop2.$doc.ajaxStart(function(){miniShop2.ajaxProgress=!0}).ajaxStop(function(){miniShop2.ajaxProgress=!1}).on('submit',miniShop2.form,function(e){e.preventDefault();var $form=$(this);var action=$form.find(miniShop2.action).val();if(action){var formData=$form.serializeArray();formData.push({name:miniShop2.actionName,value:action});miniShop2.sendData={$form:$form,action:action,formData:formData};miniShop2.controller()}});miniShop2.Cart.initialize();miniShop2.Message.initialize();miniShop2.Order.initialize();miniShop2.Gallery.initialize()};miniShop2.controller=function(){var self=this;switch(self.sendData.action){case 'cart/add':miniShop2.Cart.add();break;case 'cart/remove':miniShop2.Cart.remove();break;case 'cart/change':miniShop2.Cart.change();break;case 'cart/clean':miniShop2.Cart.clean();break;case 'order/submit':miniShop2.Order.submit();break;case 'order/clean':miniShop2.Order.clean();break;default:return}};miniShop2.send=function(data,callbacks,userCallbacks){var runCallback=function(callback,bind){if(typeof callback=='function'){return callback.apply(bind,Array.prototype.slice.call(arguments,2))}
else if(typeof callback=='object'){for(var i in callback){if(callback.hasOwnProperty(i)){var response=callback[i].apply(bind,Array.prototype.slice.call(arguments,2));if(response===!1){return!1}}}}
return!0};if($.isArray(data)){data.push({name:'ctx',value:miniShop2Config.ctx})}
else if($.isPlainObject(data)){data.ctx=miniShop2Config.ctx}
else if(typeof data=='string'){data+='&ctx='+miniShop2Config.ctx}
var formActionUrl=(miniShop2.sendData.$form)?miniShop2.sendData.$form.attr('action'):!1;var url=(formActionUrl)?formActionUrl:(miniShop2Config.actionUrl)?miniShop2Config.actionUrl:document.location.href;var formMethod=(miniShop2.sendData.$form)?miniShop2.sendData.$form.attr('method'):!1;var method=(formMethod)?formMethod:'post';if(runCallback(callbacks.before)===!1||runCallback(userCallbacks.before)===!1){return}
...
else{$.jGrowl.defaults.closerTemplate='<div>[ '+miniShop2Config.close_all_message+' ]</div>';miniShop2.Message.close=function(){$.jGrowl('close')};miniShop2.Message.show=function(message,options){if(message!=''){$.jGrowl(message,options)}}}},success:function(message){miniShop2.Message.show(message,{theme:'ms2-message-success',sticky:!1})},error:function(message){miniShop2.Message.show(message,{theme:'ms2-message-error',sticky:!1})},info:function(message){miniShop2.Message.show(message,{theme:'ms2-message-info',sticky:!1})}};miniShop2.Utils={empty:function(val){return(typeof(val)=='undefined'||val==0||val===null||val===!1||(typeof(val)=='string'&&val.replace(/\s+/g,'')=='')||(typeof(val)=='object'&&val.length==0))},formatPrice:function(price){var pf=miniShop2Config.price_format;price=this.number_format(price,pf[0],pf[1],pf[2]);if(miniShop2Config.price_format_no_zeros){price=price.replace(/(0+)$/,'');price=price.replace(/[^0-9]$/,'')}
